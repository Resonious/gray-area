...
(function(){
  var ref$, each, all, fold, foldr, filter, reject, map, empty, keys, values, abs, signum, join, x$, bodyBounds, unionify, unionExcluding, area, dimensionFrom;
  ...
  dimensionFrom = function(touching){
    var ref$;
    switch (ref$ = [touching], false) {
    case !function(it){
      return it.up || it.down;
    }(ref$[0]):
      return 'width';
    case !function(it){
      ...
      return 'height';
    default:
      throw 'bullshit liar there is no collide';
    }
  };
  this.PlatformCollision = {
    collide: curry$(function(physics, platforms, player){
      ...
      each(function(platform){
        return physics.collide(player, platform.inside, null, PlatformCollision.processInsideDifferent(player.color, platform));
      })(
      platforms);
      each(function(platform){
        return physics.collide(player, platform.inside, null, PlatformCollision.processInsideSame(player.color, platform));
      })(
      platforms);
    ...
    processInsideDifferent: function(color, platform){
      switch (false) {
    ...
    processInsideSame: function(color, platform){
      switch (false) {
      case !(!color || !platform.color):
        throw "Something went wrong with platform/player color!";
      case color !== platform.color:
        return function(player, platformInside){
          ...
          checkRect = unionExcluding(null, player);
          if (area(intersection) > playerArea * 0.9) {
            ...
            dimen = dimensionFrom(platformInside.body.touching);
